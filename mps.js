...
let data2 = {};
d3.csv("https://patrickmugayajoel.github.io/d3/mps_test.csv").then(function(data) { 
  // console.log(data)
  data.forEach((obj, index) => {
    let grp_name = obj.Constituency.split(" ")[0]
    if(data2[grp_name]){
      const already = data2[grp_name].zmap.indexOf(obj.Constituency);
      if(already+1) {
        data2[grp_name][already][Object.keys(data2[grp_name][already]).length-1] = [obj["Name"], obj["Political Party"], obj["Votes"]];
      } else {
        data2[grp_name].push({
      ...
      data2[grp_name].zmap = [obj.Constituency];
    }
    // data.splice(i, 1);
  });
